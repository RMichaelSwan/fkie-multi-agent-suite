...
export default function ActionPanel(props: ActionPanelProps): JSX.Element {
  const { showOptions, provider, defaultActionName = "", defaultActionType = "" } = props;

  const rosCtx = useRosContext();
  const logCtx = useLoggingContext();

  const [actionName] = useState(defaultActionName);
  ...
  // Use useMsgHistory for goal persistence
  ...
  // Resolve action type from provider's node list
  useEffect(() => {
  ...
  // Load goal struct when action type is known
  useEffect(() => {
  ...
  // Update input elements when goalStruct or searchTerm changes
  ...
  async function handleSendGoal(): Promise<void> {
    ...
    if (!provider || !actionName || !actionType) {
      setGoalError("Action name and type are required");
      return;
    }
  ...
  function onKeyDown(event: React.KeyboardEvent): void {
    ...
    if (event.ctrlKey && event.key === "Enter") {
      if (!isActive) handleSendGoal();
    }
  }

  return (
    <Box height="100%" overflow="auto" alignItems="center" sx={getHostStyle()} onKeyDown={onKeyDown}>
      <Stack spacing={1} margin={1}>
        ...
        <Stack direction="row" alignItems="center" spacing={1}>
          ...
          <Chip label={actionStatus} color={statusColor} size="small" variant="outlined" />
        </Stack>
