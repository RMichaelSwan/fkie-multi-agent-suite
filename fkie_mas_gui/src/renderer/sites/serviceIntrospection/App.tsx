...
import ServiceIntrospectionPanel from "@/renderer/pages/NodeManager/panels/ServiceIntrospectionPanel";
...
export default function ServiceIntrospectionApp(): JSX.Element {
  ...
  async function initProvider(): Promise<void> {
    ...
    document.title = `Introspect service - ${getFileName(serviceName)}`;
  ...
  useEffect(() => {
    window.serviceManager?.onClose(async (id: string) => {
      await appInfoRef.current?.provider.stopServiceIntrospection(appInfoRef.current.serviceName);
