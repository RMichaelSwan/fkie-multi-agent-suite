...
import ActionIntrospectionPanel from "@/renderer/pages/NodeManager/panels/ActionIntrospectionPanel";
...
export default function ActionIntrospectionApp(): JSX.Element {
  ...
  async function initProvider(): Promise<void> {
    ...
    document.title = `Introspect action - ${getFileName(serviceName)}`;
  ...
  useEffect(() => {
    window.serviceManager?.onClose(async (id: string) => {
      await appInfoRef.current?.provider.stopActionIntrospection(appInfoRef.current.serviceName);
